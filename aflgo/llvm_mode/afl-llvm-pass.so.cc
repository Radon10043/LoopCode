...
/* args */
cl::opt<std::string> LoopBBsFile(
    "loopfile",
    cl::desc("LoopBBs.txt"),
    cl::value_desc("loopfile"));
...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  std::string loopBB;
  std::vector<std::string> loopBBVec; // 存储循环BB名字的vector
  int loopBBNum = 0;
  if (fin.is_open()) {
    while (getline(fin, loopBB)) {
      loopBBVec.push_back(loopBB);
    }
    loopBBNum = loopBBVec.size();
  } else errs() << "I can't find LoopBBs.txt\n";
  ...
  if (is_aflgo_preprocessing) {
    ...
    std::ofstream instrLog("/home/radon/Documents/instrLog.txt", std::ofstream::out | std::ofstream::app);
    ...
    for (auto &F : M) {
      ...
      for (auto &BB : F) {
        ...
        std::string bb_name_copy; // 拷贝一份基本块名
        ...
        if (is_aflgo) {
          ...
          for (auto &I : BB) {
            ...
            bb_name = filename + ":" + std::to_string(line);
            // instrLog << "bbname: " << bb_name << "\n";

            bb_name_copy = bb_name; // 这里的bb_name是局部变量, 所以将aflgo获得的基本块名拷贝一份
        ...
        if (idx < loopBBNum && instrLoopBBs.insert(bb_name_copy).second) {
          ...
          Value* MapLoopPtr = IRB.CreateBitCast(
              IRB.CreateGEP(MapPtr, MapLoopLoc), LargestType->getPointerTo());
          LoadInst *MapLoop = IRB.CreateLoad(MapLoopPtr);
          MapLoop->setMetadata(M.getMDKindID("nonsanitize"), MDNode::get(C, None));

          Value *IncrLoop = IRB.CreateAdd(MapLoop, One);
          IRB.CreateStore(IncrLoop, MapLoopPtr)
              ->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
        }
