...
static u32 stop_time = 60;            /* Fuzzing time (Default: 60 min) */
static u64 loop_cov_cnt[MAX_RECORD_LOOP_NUM] = {0};   /* 统计覆盖到循环BB测试用例数量的数组 */
static u64 loop_max_times[MAX_RECORD_LOOP_NUM] = {0}; /* 统计每个循环的最大循环次数 */
...
static inline u8 has_new_bits(u8* virgin_map) {
  ...
  /* 读取共享内存, 看看覆盖到了哪些循环 */
  for (u16 j = 0; j < MAX_RECORD_LOOP_NUM; j++) {
    u64 loop_times = *(u64*)(trace_bits + MAP_SIZE + 16 + j * 8); //读取该测试用例触发的循环次数
    loop_max_times[j] = loop_max_times[j] > loop_times ? loop_max_times[j] : loop_times;  // 更新最大循环次数
    if (loop_times)
      loop_cov_cnt[j]++;  // 记录覆盖到循环的测试用例数量
  }
  ...
  if (get_cur_time() - start_time > stop_time * 60000) {
    ...
    u8* loop_data_path;
    loop_data_path = alloc_printf("%s/loop_recorded", out_dir);
    FILE* f = fopen(loop_data_path, "a+");

    for (u16 j = 0; j < MAX_RECORD_LOOP_NUM; j++)
      fprintf(f, "%llu-%llu\n", loop_cov_cnt[j], loop_max_times[j]);   // 覆盖到该循环的测试用例数量-最大循环次数
    fclose(f);
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "  -k keeping    - fuzzing time (Default: 60 min)\n\n"
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Qz:c:k:")) > 0) // Rn: 添加了 -k 表示持续模糊测试的时间
