...
def k_means_main(seed_path=None, out_path=None):
    ...
    4. 获得特征标签
    5. 从每个簇种随机选择种子，并将选出的种子文件名存入文件seed_name中
    ...
    for file in files:  # 遍历文件夹
        if not os.path.isdir(file) and not file.endswith('.txt'):  # 判断是否是文件夹，不是文件夹才打开。并且不以.txt结尾
    ...
    """5. 从每个簇中随机选择种子，并将选出的种子文件名存入文件seeds_name中"""
...
k_means_main(r'C:\Users\59489\PycharmProjects\聚类算法\queue', r'C:\Users\59489\PycharmProjects\聚类算法\good_seeds.txt')
