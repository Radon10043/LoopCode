...
#$PY_PATH -u $KEEP_SHOWMAP_THREAD_PATH --kmeans F --fuzzer-stats $FUZZER_STATS --log-path $PY_OUTPUT_DIR_PATH --out-path $TMP_DIR --skip-log-stdout --good-seeds-path $GOOD_SEEDS_PATH --gcc-version-bin /home/lowry/Documents/LoopCode/scripts/jasper-2.0.21-gcc/obj-loop/src/app/jasper --append-args "--output /tmp/out_afl_origin.jpg --input" --testcase-dir-path $SUBJECT/obj-loop/out & # 后台运行py
#$PY_PATH -u $KEEP_SHOWMAP_THREAD_PATH --kmeans F --fuzzer-stats $FUZZER_STATS --log-path $PY_OUTPUT_DIR_PATH --out-path $TMP_DIR --skip-log-stdout --good-seeds-path $GOOD_SEEDS_PATH --gcc-version-bin /home/lowry/Documents/LoopCode/scripts/jasper-3.0.3-gcc/obj-loop/src/app/jasper --append-args "--output /tmp/out_afl_origin.jpg --input" --testcase-dir-path $SUBJECT/obj-loop/out & # 后台运行py
#sleep 5s
